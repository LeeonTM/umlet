...
public class SharedUtils {
	...
	public static Rectangle realignToGrid(Rectangle rectangle, boolean realignHeightAndWidth) {
		...
		if (realignHeightAndWidth) {
			int width = realignToGrid(false, rectangle.getX() - (double) x + rectangle.getWidth(), true); // IMPORTANT: the difference from original x and realigned x must be added, otherwise the upper example would return x=0, width=10, x2=10. x2 would be too small
			int height = realignToGrid(false, rectangle.getY() - (double) y + rectangle.getHeight(), true);
